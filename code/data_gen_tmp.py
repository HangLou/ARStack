...
# for name in os.listdir('train_preds\\'):
# if 'alpha' in name:
# if name[:4] in selection:
# ls.append(name[:4])
...
def superimpose(alpha_atom_coordinates, baker_atom_coordinates, ref_atom_coordinates):
    ...
    baker_trans = get_transformed_coordinates(ref_array, baker_array)
...
def main(dataset=1):
    ...
    for protein in prot_ls:  # [:5]:
        if protein == '4B3W':
            ...
            try:
                ...
                base_baker_atom_coordinates = get_baker_coordinates(
                    protein, dataset)

                alpha_prediction = get_alpha_ppsequence_and_coordinates(
                    protein, dataset)
                ...
                alignment_outcome = align_alpha_reference_seqs(
                    protein, alpha_polypeptide_sequence, dataset=dataset)
                ...
                print(len(alpha_backbone_indices))
            ...
            #"""
                ...
                total.append([protein]+distances)
                # input(total)
                atom_counts.append([protein]+alpha_backbone_indices)
                vector_data.append([protein]+vector_info)
    ...
    print('done')
    return(total, atom_counts, vector_data)


if __name__ == '__main__':
    total, atom_counts, vector_data = main()
    print('harvested')

    df = pd.DataFrame(total)
    df2 = pd.DataFrame(atom_counts)
    df3 = pd.DataFrame(vector_data)

    # df.to_csv('JK_two_per_fam_train_target_values.csv') #target values (distances to walk from alpha coord on the alpha->baker vector closest to the experimental coordainte)
    # df2.to_csv('JK_two_per_fam_train_atom_counts.csv') # atoms involved (only aligned atoms)
    # df3.to_csv('JK_two_per_fam_train_vector_info.csv') # vector info necessary to recover corodinate from predicted target values
